    ...
    public static class Helpers
    {
        ...
        public static bool GetSystem()
        {
            ...
            if (IsHighIntegrity())
            {
                ...
                bool success = Interop.OpenProcessToken(handle, 0x0002, out var hToken);
                if (!success)
                {
            ...
            for (int i = 0; i < number.ToInt32(); i++)
            {
                ...
                if (classVal.ToString().StartsWith("0"))
                {
                    ...
                    if (v[ntHashOffset + 2].Equals(0x01))
                    {
                        ...
                        if (ntHashLength == 20)
                        {
                            var encryptedNtHash = v.Skip(ntHashOffset + 4).Take(16).ToArray();
                        ...
                        if (lmHashLength == 20)
                        {
                            var encryptedLmHash = v.Skip(lmHashOffset + 4).Take(16).ToArray();
        ...
        public static byte[] GetRegKeyValue(string keyPath, string valueName = "", bool silent = false)
        {
            IntPtr hKey = IntPtr.Zero;
        ...
        public static bool IsHighIntegrity()
        {
            ...
            if (identity.ImpersonationLevel == TokenImpersonationLevel.Impersonation || identity.ImpersonationLevel == TokenImpersonationLevel.Delegation)
            {
